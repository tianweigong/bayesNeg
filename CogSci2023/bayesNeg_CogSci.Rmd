...
#   mutate(cls=substr(sub,1,2),
#          sd_l=NA,sd_r=NA,gamma_a=NA,gamma_b=NA,sd_center=NA,LEW=NA,REW=NA,EWB=NA,EWS=NA,DBE=NA)
# 
...
#          ) %>%
#   select(sub,cls,sd_l,sd_r,sd_center,gamma_a,gamma_b,line_prior,line_prior_var,LEW,REW,EWB,EWS,DBE) %>%
#   dplyr::rename(sigL=sd_l,sigR=sd_r,sigC=sd_center,alpha=gamma_a,beta=gamma_b,
#          line_var=line_prior_var)
```

```{r}
# df.fit.sym=data.frame(sub=unique(df.all$sub))%>%
#   mutate(cls=substr(sub,1,2),
#          sd_l=NA,sd_r=NA,gamma_a=NA,gamma_b=NA,sd_center=NA,LEW=NA,REW=NA,EWB=NA,EWS=NA,DBE=NA)
# 
# for (k in 1:nrow(df.fit.sym)){
#   load(paste("fit_symmetric/",df.fit.sym$sub[k],".Rda",sep=""))
# 
#   df.fit.sym$sd_l[k]=post_lis$sd_l[[1]]
#   df.fit.sym$sd_r[k]=post_lis$sd_r[[1]]
#   df.fit.sym$gamma_a[k]=post_lis$gamma_a[[1]]
#   df.fit.sym$gamma_b[k]=post_lis$gamma_b[[1]]
#   df.fit.sym$sd_center[k]=post_lis$sd_center[[1]]
# }
# 
# for (k in 1:nrow(df.fit.sym)){
#   d1=df.all %>% subset(sub==df.fit.sym$sub[k]) %>% subset(L==-R)
#   md=lm(ans~end_l,d1)
# 
#   df.fit.sym$LEW[k]=-as.numeric(md$coefficients[2])
#   # df.fit.sym$REW[k]=as.numeric(md$coefficients[3])
#   df.fit.sym$EWB[k]=-as.numeric(md$coefficients[2])
#   # df.fit.sym$EWS[k]=as.numeric(md$coefficients[3]+md$coefficients[2])
#   
#   df.fit$DBE[k]=mean(d1$ans-(d1$end_l+d1$end_r)/2)
# }
# 
# df.fit.sym=df.fit.sym %>%
#   mutate(line_prior=gamma_a/gamma_b,
#          line_prior_var=gamma_a/(gamma_b)^2,
#          cls=factor(cls,levels = c("NC","VN"))
#          ) %>%
...
#          line_var=line_prior_var)
# 
# save(df.fit,df.fit.sym,file="humanFit.Rda")
```
...
p3=df.fit %>%
  ...
  geom_hline(yintercept = 0.062,linetype="dashed")+
...
p4=df.fit %>%
  ...
  geom_hline(yintercept = 0.016,linetype="dashed")+
  geom_vline(xintercept = 0.014,linetype="dashed")+
  ylab("Endpoint Weightings Bias (EWB)")
...
```{r}
myVPrior<-function(sub,nam,lengthout=100){
  ...
  if (nam=="Patient (Strong Prior, 50%)"){
    ...
    s=seq(0,1.5,length.out=lengthout)
  }
  df.line=data.frame()
  ...
  df.line %>%
    ggplot(aes(x=line_length*2,y=prob,group=sub))+
    facet_wrap(~cls)+
    geom_line(alpha=0.3,color="#b51700")+
...
g3=df.fit$sub[which(df.fit$line_var<1e-3)] 
g2=setdiff(df.fit$sub[which(df.fit$cls=="VN")],
           df.fit$sub[which(df.fit$line_var<1e-3)])

myVPrior(g1,"Healthy Control")+
  scale_y_continuous(limits = c(0,3.3))

# ggsave(file="p_h.pdf",width = 3.8,height = 4)

myVPrior(g2,"Patient (50%)")+
    scale_y_continuous(limits = c(0,3.3))

# ggsave(file="p_p.pdf",width = 3.8,height = 4)

myVPrior(g3,"Patient (Strong Prior, 50%)")+
...
sd(df.fit$line_prior[df.fit$cls=="NC"]*2)

t.test(line_var~cls, df.fit)
t.test(line_var~cls, df.fit[!df.fit$sub %in% g3,])
...
myLinePlot<-function(p){
  ...
  ggplot() +
    ...
    geom_line(data=data.frame(y=c(0.2-0.01,0.2+0.01)),aes(x=0,y=y),color="#b51700",linetype="longdash",size=0.3)+
      geom_line(data=data.frame(y=c(0.15-0.01,0.15+0.01)),aes(x=-0.25,y=y),color="#b51700",linetype="longdash",size=0.3)+
      geom_line(data=data.frame(y=c(0.1-0.01,0.1+0.01)),aes(x=0.25,y=y),color="#b51700",linetype="longdash",size=0.3)+
        geom_line(data=data.frame(y=c(0.05-0.01,0.05+0.01)),aes(x=0,y=y),color="#b51700",linetype="longdash",size=0.3)+
...
# ggsave(file="p_vn46.pdf",width = 2,height = 2.5)
...
myLinePriorPlot("VN46")+
  scale_y_continuous(breaks = c(0,10000),
                   labels = c(0,expression (10^4)))
# ggsave(file="p_vn46_p.pdf",width = 2,height = 2.5)
...
cor(df.fit$sigL,df.fit$line_var)
```
...
df.cor=merge(select(df.fit,sub,DBE,EWB,sigL),
...
predlis=list(c("DBE"),
             c("LEW"),
             c("LEW","REW"),
             c("sigL"),c("sigL","sigR"),c("sigL","sigR","line_prior","line_var"),c("sigL","sigR","sigC"),c("sigL","sigR","line_prior","line_var","sigC"), #combination of rational factors
             c("sigL","LEW"),
             c("sigL","sigR","LEW","REW")
              )
